...
import { GameSnapshot, Card } from '../engine/types';
...
export function botDecideBet(
  ...
  if (!playerData || validBets.length === 0) return validBets[0] ?? 0;
  ...
  if (validBets.includes(estimate)) return estimate;

  // Buscar el valor más cercano a la estimación que sea válido
  const closest = validBets.reduce((best, v) =>
    Math.abs(v - estimate) < Math.abs(best - estimate) ? v : best,
    validBets[0],
  );
...
export function botDecideCard(
  ...
  const playerData = snapshot.playerData.find(p => p.playerId === botId);
  if (!playerData || playerData.hand.length === 0) return 0;
 ...
 */
export function botThinkTime(baseMs = 1200): number {
