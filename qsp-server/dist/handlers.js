...
function registerHandlers(io, socket) {
    const playerId = (0, uuid_1.v4)(); // UUID único por conexión
    // ── SALA: CREAR ───────────────────────────────────────────────
    socket.on('room:create', (payload, cb) => {
        try {
            const room = roomManager.createRoom(socket.id, playerId, payload.playerName, payload.avatarId, payload.colorId, payload.size);
    ...
    socket.on('room:join', (payload, cb) => {
        const result = roomManager.joinRoom(socket.id, playerId, payload.playerName, payload.avatarId, payload.colorId, payload.code);
        if (result.error) {
    ...
    socket.on('room:quick', (payload, cb) => {
        const room = roomManager.quickMatch(socket.id, playerId, payload.playerName, payload.avatarId, payload.colorId, payload.preferredSize ?? 4);
        socket.join(room.code);
        cb({ ok: true, data: {
                roomCode: room.code,
                yourPlayerId: playerId,
    ...
    socket.on('room:start', (...args) => {
        ...
        if (!room) {
            cb({ ok: false, error: 'No estás en ninguna sala.' });
            return;
        }
        ...
        if (room.players.filter(p => !p.isBot).length < 1) {
            cb({ ok: false, error: 'Se necesitan al menos 2 jugadores.' });
            return;
        }
    ...
    socket.on('room:leave', (cb) => {
        ...
        if (!result) {
            cb({ ok: false, error: 'No estás en una partida.' });
            return;
        }
    ...
    socket.on('room:rematch', (cb) => {
        ...
        if (!result) {
            cb({ ok: false, error: 'Error al votar revancha.' });
            return;
        }
        cb({ ok: true, data: {} });
        if (result.allVoted) {
    ...
    socket.on('game:bet', (payload, cb) => {
        const room = roomManager.getRoomBySocket(socket.id);
        if (!room) {
            cb({ ok: false, error: 'No estás en una sala.' });
            return;
        }
        const session = gameSessions.get(room.code);
        if (!session) {
            cb({ ok: false, error: 'El juego no está activo.' });
            return;
        }
        const result = session.placeBet(playerId, payload.bet);
        if (!result.success) {
            cb({ ok: false, error: result.reason ?? 'Error al apostar.' });
            return;
        }
        cb({ ok: true, data: {} });
    ...
    socket.on('game:play', (payload, cb) => {
        ...
        if (!result.success) {
            cb({ ok: false, error: result.reason ?? 'Movimiento inválido.' });
            return;
        }
        cb({ ok: true, data: {} });
    ...
    socket.on('game:message', (payload) => {
        ...
        if (!room)
            return;
        const msgData = events_1.QUICK_MESSAGES[payload.messageId];
        if (!msgData)
            return;
...
function startGame(io, roomCode) {
    ...
    const session = new GameSession_1.GameSession(room);
    gameSessions.set(roomCode, session);
    session.start();
    setTimeout(() => {
        io.to(roomCode).emit('room:started', session.toPublicSnapshotDTO());
...
function broadcastSnapshots(io, roomCode, session) {
    const room = roomManager.getRoom(roomCode);
    if (!room)
        return;
...
function handleRoundEnd(io, roomCode, session) {
...
function scheduleBotBet(io, roomCode, session) {
    const snap = session.getSnapshot();
    if (snap.state !== 'BETTING')
        return;
    ...
    const validBets = session.getValidBets();
    setTimeout(() => {
        const bet = (0, bot_1.botDecideBet)(bettorId, session.getSnapshot(), validBets);
    ...
    }, (0, bot_1.botThinkTime)());
}
/** Programa la jugada de un bot si es su turno */
function scheduleBotActions(io, roomCode, session) {
    const snap = session.getSnapshot();
    ...
    if (!currentPlayer?.isBot)
        return;
    setTimeout(() => {
        const cardIndex = (0, bot_1.botDecideCard)(currentPlayerId, session.getSnapshot());
